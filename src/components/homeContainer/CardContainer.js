...
import {Teaser, AnimeData, MoviesData, MoviesDataTrailer, WebSeriesData } from '../dataContainer/Data';
...
const YouTubePlayList = ({ title, linkPage, linkPlay, movies }) => {
  const [slidePositions, setSlidePositions] = useState(0);
      ...
      <div className={`${style.bottomContainer}`}>
        ...
        {videos.map((movie, index) => (
          <>
            ...
            <img className={style.playButton} src={Play} alt="playButton" />
              <Link to={`/video/${movie.snippet.resourceId.videoId}`}>
                <div className={style.mobileView}>
              ...
              <Link to={`/video/${movie.snippet.resourceId.videoId}`}>
                  ...
                  <iframe className={`${style.cardIframe} ${style.youTubeIframe}`} src={`https://www.youtube.com/embed/${movie.snippet.resourceId.videoId}`} frameBorder="0" allow='autoplay'
...
// YouTube Movies
const YouTubePlayMoviesList = ({ title, linkPage, linkPlay, movies }) => {
  const [slidePositions, setSlidePositions] = useState(0);
  const [hoveredImageIndex, setHoveredImageIndex] = useState(null);
  const [videos, setVideos] = useState([]);
...
const YouTubePlayTVList = ({ title, linkPage, linkPlay, movies }) => {
  const [slidePositions, setSlidePositions] = useState(0);
  const [hoveredImageIndex, setHoveredImageIndex] = useState(null);
  const [videos, setVideos] = useState([]);
  ...
  return (
    <div className={style.cardContainer}>
      ...
      <div className={`${style.bottomContainer} ${style.mrBeastBottomContainer} ${style.notReady}`}>
        {/* <img
          className={style.backwordArrow}
          src={BackwardArrowImage}
          onClick={() => handleSlide('left')}
          ...
          <>
            ...
            {hoveredImageIndex === movie.id && (
              <Link to={`/videoMovies/${movie.snippet.resourceId.videoId}`}>
                  ...
                  {/* <iframe className={`${style.cardIframe} ${style.mrBeastCardIframe}`} src={`https://www.youtube.com/embed/${movie.snippet.resourceId.videoId}`} frameBorder="0" allow='autoplay'
                  allowFullScreen></iframe> */}
                  <div>
...
const YouTubeClip = ({ title, linkPage, linkPlay, movies }) => {
  const [slidePositions, setSlidePositions] = useState(0);
  const [hoveredImageIndex, setHoveredImageIndex] = useState(null);
  const [videos, setVideos] = useState([]);
          ...
          <>
            ...
            {hoveredImageIndex === movie.id && (
              <Link to={`${linkPlay}/${movie.id}`}>
                  ...
                  {/* <iframe className={style.cardIframe} src={movie.video} frameBorder="0" allow='autoplay'></iframe> */}
                  <div>
...
const CardContainer = () => {
  return (
    <div className={style.collectionOfCardContainer}>
      ...
      <YouTubePlayList title="YouTube" linkPage="/youTubeVideo" linkPlay="/youTubeLink" />
      <YouTubePlayMoviesList title="Movies" linkPage="/youTubeMovies" linkPlay="/youTubeLink" />
      <YouTubePlayTVList title="TV" linkPage="/youTubeMovies" linkPlay="/youTubeLink" />
      <YouTubeClip title="Clip" linkPage="/youTubeMovies" linkPlay="/youTubeLink" />
